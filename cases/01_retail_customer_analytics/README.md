...
이 사례는 Golden Data Lab의 [9단계 표준 분석 방법론](../../docs/methodology.md)을 따른다. 현재는 1단계 `Business Question`과 2단계 `SQL Extraction`의 분석 계약을 확정한다.
...
| 2. SQL Extraction | 확정 | PostgreSQL 원천 테이블, 기본 검증 쿼리, 분석용 추출 계약 |
| 3. Data Quality Check | 초안 작성 | `01_data_quality_check.ipynb` |
| 4~9. 후속 단계 | 예정 | EDA, 통계, KPI, BI, 제안, 재현 문서 |
...
다음 작업은 2단계 계약에 맞춰 PostgreSQL 적재 방식과 SQL 파일을 작성하는 것이다. SQL 결과가 확정된 후 `notebooks/01_data_quality_check.ipynb`를 3단계 입력 계약에 맞게 연결한다.
